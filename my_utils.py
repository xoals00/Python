...
def connect_mysql(db='lol'):
    conn = pymysql.connect(host='localhost', port=3306, user='icia', password='1234', db=db, charset='utf8')
    return conn
        ...
        try:
            if df.iloc[i].matches['info']['gameDuration'] > 900:   # 게임시간이 15분이 안넘을경우에는 패스하기
                for j in range(10):
                    ...
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['allInPings']) 
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['assistMePings'])
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['baitPings'])
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['basicPings'])
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['commandPings'])
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['dangerPings'])
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['enemyMissingPings'])
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['getBackPings'])
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['holdPings'])
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['needVisionPings'])
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['onMyWayPings'])
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['pushPings'])
                    tmp.append(df.iloc[i].matches['info']['participants'][j]['visionClearedPings'])
            ...
            print(i)
            continue
    columns = ['gameId','gameDuration','summonerName','summonerLevel','participantId','championName','champExperience', 'allInPings','assistMePings','baitPings','basicPings','commandPings','dangerPings','enemyMissingPings','getBackPings','holdPings','needVisionPings','onMyWayPings','pushPings','visionClearedPings','teamPosition','teamId','win','kills','deaths','assists','totalDamageDealtToChampions','totalDamageTaken','g_2','g_3','g_4','g_5','g_6','g_7','g_8','g_9','g_10','g_11','g_12','g_13','g_14','g_15','g_16','g_17','g_18','g_19','g_20']
...
def insert_matches_timeline_mysql(row,conn):
    ...
    query = (
             f'insert into lol_datas(gameId, gameDuration, summonerName, summonerLevel, participantId,'
             f'championName, champExperience,allInPings,assistMePings,baitPings,basicPings,commandPings,dangerPings,enemyMissingPings,getBackPings,holdPings,needVisionPings,onMyWayPings,pushPings,visionClearedPings,teamPosition, teamId, win, kills, deaths, assists,'
             f'totalDamageDealtToChampions, totalDamageTaken, g_2, g_3, g_4, g_5, g_6, g_7, g_8, g_9, g_10, g_11, g_12 ,g_13,g_14,'
             f'g_15, g_16, g_17, g_18, g_19, g_20)'
             f'values(\'{row.gameId}\',{row.gameDuration},  \'{row.summonerName}\','
             f'{row.summonerLevel}, {row.participantId},\'{row.championName}\',{row.champExperience},{row.allInPings},{row.assistMePings},{row.baitPings},{row.basicPings},{row.commandPings},{row.dangerPings},{row.enemyMissingPings},{row.getBackPings},{row.holdPings},{row.needVisionPings},{row.onMyWayPings},{row.pushPings},{row.visionClearedPings}'
             ...
             f'gameId = \'{row.gameId}\', gameDuration = {row.gameDuration}, summonerName= \'{row.summonerName}\','
             ...
             f'totalDamageTaken = {row.totalDamageTaken},g_2 = {row.g_2}, g_3 = {row.g_3}, g_4 = {row.g_4}, g_5 = {row.g_5}, g_6 = {row.g_6},g_7 = {row.g_7},g_8 = {row.g_8},g_9 = {row.g_9},'
             f'g_10 = {row.g_10},g_11 = {row.g_11},g_12 = {row.g_12},g_13 = {row.g_13},g_14 = {row.g_14},g_15 = {row.g_15},g_16 = {row.g_16},g_17 = {row.g_17},'
             f'g_18 = {row.g_18},g_19 = {row.g_19},g_20 = {row.g_20}'
            )
